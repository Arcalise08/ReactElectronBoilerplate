export const SET_WINDOW = "SET_WINDOW";
export const SET_MESSAGES = "SET_MESSAGES";
export const ADD_MESSAGE = "ADD_MESSAGES";
export const SET_NAVIGATION = "SET_NAVIGATION";
export const CLEAR_MESSAGES = "CLEAR_MESSAGES";
export const SET_COLORS = "SET_COLORS";
export const SET_SIGNAL_R = "SET_SIGNAL_R";
export const SET_BASE_API_URL = "SET_BASE_API_URL";
export const SET_DIM_BACKGROUND = "SET_DIM_BACKGROUND";
export const SET_POP_UP = "SET_POP_UP";

export const SET_THREAD_REQUESTS = "SET_THREAD_REQUESTS";

export const SET_SAVED_REQUESTS = "SET_SAVED_REQUESTS";

export const SET_SAVED_RESULTS = "SET_SAVED_RESULTS";
export const OVERRIDE_SAVED_RESULTS = "OVERRIDE_SAVED_RESULTS";
export const SET_QUEUED_MESSAGES = "SET_QUEUED_MESSAGES";
